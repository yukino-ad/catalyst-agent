    ...
    async (refreshRemote = false) => {
      ...
      try {
        ...
        if (rejected?.status === "rejected") {
          setError(monitorErrorMessage(rejected.reason));
        }
      } catch (reason) {
        setError(monitorErrorMessage(reason));
      } finally {
...
function monitorErrorMessage(reason: unknown) {
  const detail = reason instanceof Error ? reason.message : String(reason);
  if (/timed?\s*out|timeout|超时/i.test(detail)) {
    return "监控查询暂时超时，继续保留上次状态；这不会把作业视为完成或失败。";
  }
  return `部分作业暂时无法刷新，继续显示上次状态：${detail}`;
}
