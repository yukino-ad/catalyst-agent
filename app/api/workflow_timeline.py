...
STAGE_DEFINITIONS: tuple[dict[str, Any], ...] = (
    ...
    {"stage_id": "C11", "node_ids": ("dft_execution_options", "dft_local_preflight", "cluster_readonly_preflight", "remote_execution_plan", "remote_upload_review", "remote_upload", "remote_submission_review", "remote_submission", "submission_record"), "label": "提交和监控 DFT", "group": "C", "progress": 98, "next": "C12.1", "review_types": ("dft_execution_options_required", "remote_upload_review_required", "remote_submission_review_required", "result_download_review_required")},
    {"stage_id": "C12.1", "node_ids": ("adsorption_reaction_planning", "adsorption_intermediate_review"), "label": "选择吸附中间体", "group": "C12", "progress": 60, "next": "C12.2", "review_types": ("adsorption_intermediate_review_required",)},
    ...
    {"stage_id": "C12.6", "node_ids": ("adsorption_dft_execution_options", "source_filter", "monitor", "completion", "result_ready"), "label": "监控吸附 DFT", "group": "C12", "progress": 88, "next": "C12.7", "review_types": ("adsorption_dft_execution_required", "result_download_review_required")},
...
def mark_stage_waiting(
    ...
    *,
    ...
    """Keep an externally running DFT stage open after graph handoff."""
    now = datetime.now(timezone.utc).isoformat()
    updated = deepcopy(timeline)
    for stage in updated:
        if stage.get("stage_id") != stage_id:
        ...
        stage["updated_at"] = now
        stage["summary"] = summary
        stage["requires_human_action"] = False
        stage["skip_reason"] = ""
        stage["error"] = ""
        if outputs is not None:
            stage["outputs"] = deepcopy(outputs)
...
def mark_node_update(
    ...
    summary: str = "",
    outputs: dict[str, Any] | None = None,
    stage_ids_override: list[str] | None = None,
) -> list[dict[str, Any]]:
    stage_ids = stage_ids_override if stage_ids_override is not None else stage_ids_for_node(node_id)
