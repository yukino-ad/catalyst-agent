...
class TaskManager:
    def __init__(
        ...
        submitted_job_repository: Any | None = None,
        graph: Any | None = None,
        executor: Any | None = None,
    ) -> None:
        self.repository = repository or WorkflowRunRepository()
        self._submitted_job_repository = submitted_job_repository
        self._graph = graph
    ...
    def resume_if_ready(self, task_id: str) -> bool:
        """Start the persisted post-DFT graph once every active job is terminal."""
        ...
        with self._resume_lock:
            ...
            if not isinstance(record, dict):
                return False
            if record.get("workflow_status") in {
                "waiting_for_human", "resuming_dft_results", "running_resume"
            }:
                return False
            try:
                target = resolve_resume_target(record)
                ...
                return False
            if target not in {"job_operations", "adsorption_monitoring"}:
                return False
            identifiers = [str(value) for value in record.get("active_slurm_jobs", [])]
            jobs = self._submitted_job_repository or SubmittedJobRepository()
            persisted = [jobs.get(value) for value in identifiers]
            if not identifiers or any(not isinstance(item, dict) for item in persisted):
                return False
            if not all(
                bool(item.get("terminal"))
                or str(item.get("scheduler_state", "")) in {
            ...
            self.repository.update(task_id, {
                "workflow_status": "resuming_dft_results",
                "message": "超算作业已结束，正在进入结果审查与后续工作流。",
            })
            self._executor.submit(self._run_resume_target, task_id, target)
            return True
    ...
    def _resume(self, task_id: str, decision: dict[str, Any]) -> None:
        try:
            ...
            target = str(record.get("execution_target", "main"))
            thread_id = str(record.get("execution_thread_id", "")) or task_id
            graph = self._graph_for_target(target)
            config = {"configurable": {"thread_id": thread_id}}
            self._consume(
                task_id, graph, config, Command(resume=decision),
                execution_target=target,
            )
        except Exception as error:
            self._record_failure(task_id, error)

    def _run_resume_target(self, task_id: str, target: str) -> None:
        try:
            from app.workflow_resume_cli import _adsorption_energy_state, _submission_state

            workflow = self.repository.get(task_id) or {}
            thread_id = (
                f"web-resume-{task_id}-{target}-"
                f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
            )
            if target == "job_operations":
                initial_state = {
                    "slurm_job_ids": [str(value) for value in workflow.get("active_slurm_jobs", [])],
                    "task_id": task_id,
                    "errors": [],
                    "warnings": [],
            ...
            elif target == "adsorption_monitoring":
                initial_state = {
                    "requested_slurm_job_ids": [str(value) for value in workflow.get("active_slurm_jobs", [])],
                    "task_id": task_id,
                    "errors": [],
                    "status": "workflow_resume_created",
                }
                ...
                raise ValueError(f"Unsupported web resume target: {target}")
            self.repository.update(task_id, {
                "workflow_status": "running_resume",
                "execution_target": target,
                "execution_thread_id": thread_id,
                "message": "正在恢复计算结果处理与后续工作流。",
            })
            self._consume(
                task_id,
                self._graph_for_target(target),
        ...
        *,
        ...
        with self._lock:
            for chunk in graph.stream(graph_input, config=config, stream_mode="updates"):
                ...
                for node in chunk:
                    ...
                    stage_override = self._stage_override(execution_target, node)
                    ...
                    if stage_override:
                        display_stage = next(
                            (
                                stage for stage in timeline
                                if stage.get("stage_id") == stage_override[-1]
                            ),
                            None,
                        )
                        if display_stage:
                            label = str(display_stage.get("stage_label", label))
                            progress = int(display_stage.get("progress", progress))
                    workflow_handoff = self._workflow_handoff(
                        chunk.get(node, {}), self.repository.get(task_id) or {}
                    )
                    if workflow_handoff.get("workflow_status") == "waiting_for_dft_results":
                        waiting_stage = (
                            "C12.6"
                            if str(workflow_handoff.get("resume_stage", "")).startswith("c12.6")
                            else "C11"
                        )
                        timeline = mark_stage_waiting(
                            timeline,
                            waiting_stage,
                            summary="DFT 作业已提交，正在等待超算计算完成。",
                    ...
                    update = {
                        "workflow_status": str(workflow_handoff.get("workflow_status") or "running"),
                        "stage": node,
                        "stage_label": label,
                        "progress": progress,
                        "waiting_for_human": False,
                    ...
                    for key in ("active_slurm_jobs", "resume_stage", "last_completed_stage", "terminal"):
                        if key in workflow_handoff:
                            update[key] = workflow_handoff[key]
                    self.repository.update(task_id, update)
        ...
        if last_interrupt:
            ...
            review_stage = (
                "C12.6"
                if (
                    execution_target == "adsorption_submission"
                    and review_type in c12_execution_reviews
                )
                or (
                    execution_target == "adsorption_monitoring"
                    and review_type == "result_download_review_required"
                )
                else stage_id_for_review(review_type)
            )
            if review_stage:
                timeline = update_stage(
                    timeline,
            ...
            self.repository.update(task_id, {
                ...
                "execution_thread_id": str(config.get("configurable", {}).get("thread_id", "")),
            })
            return

        stop_reason = str(values.get("workflow_stop_reason", "") or "")
        current = self.repository.get(task_id) or {}
        if self._preserve_nonterminal_handoff(task_id, current):
            return
    ...
    @staticmethod
    def _workflow_handoff(output: Any, persisted: dict[str, Any]) -> dict[str, Any]:
        if isinstance(output, dict) and isinstance(output.get("workflow_run"), dict):
            workflow = output["workflow_run"]
            if workflow.get("workflow_status"):
                return workflow
        if persisted.get("workflow_status") == "waiting_for_dft_results":
            return persisted
        return {}

    def _preserve_nonterminal_handoff(
        self, task_id: str, current: dict[str, Any]
    ) -> bool:
        if current.get("terminal") is True or not current.get("resume_stage"):
            return False
        stage = str(current.get("resume_stage", ""))
        status = str(current.get("workflow_status", ""))
        if status == "waiting_for_dft_results" or stage in {
            "c11.5.2_clean_slab_result_monitoring",
            "c12.6_adsorption_result_monitoring",
        }:
            timeline_stage = "C12.6" if stage.startswith("c12.6") else "C11"
            timeline = mark_stage_waiting(
                current.get("workflow_timeline") or create_timeline(),
                timeline_stage,
                summary="DFT 作业仍在超算运行；完成后将继续结果审查。",
            )
            self.repository.update(task_id, {
                "workflow_status": "waiting_for_dft_results",
                "stage": stage,
                "stage_label": "等待超算 DFT 结果",
                "progress": 88 if timeline_stage == "C12.6" else 98,
                "waiting_for_human": False,
                "review_type": "",
                "review": {},
                "message": "DFT 作业正在运行，C12 不会提前跳过。",
                "workflow_timeline": timeline,
            })
            return True
        try:
            ...
            target = resolve_resume_target(current)
        except ValueError:
            return False
        if target in {"job_operations", "adsorption_submission", "adsorption_monitoring", "adsorption_energy"}:
            self.repository.update(task_id, {
                "workflow_status": "queued_resume",
                "message": "上一计算阶段已完成，正在连接下一阶段。",
            })
            self._executor.submit(self._run_resume_target, task_id, target)
            return True
        return False

    def _graph_for_target(self, target: str) -> Any:
        if target in {"", "main"}:
            return self._get_graph()
        if target == "job_operations":
            from app.graph.job_operations import job_operations_graph
            return job_operations_graph
        if target == "adsorption_submission":
            from app.graph.adsorption_execution_workflow import adsorption_execution_graph
            return adsorption_execution_graph
        if target == "adsorption_monitoring":
            from app.graph.adsorption_job_operations import adsorption_job_operations_graph
            return adsorption_job_operations_graph
        if target == "adsorption_energy":
            from app.graph.adsorption_job_operations import adsorption_energy_resume_graph
            return adsorption_energy_resume_graph
        raise ValueError(f"Unsupported execution target: {target}")

    @staticmethod
    def _stage_override(target: str, node: str) -> list[str] | None:
        result_nodes = {"monitor", "completion", "download_review", "download", "parse"}
        adsorption_submission_nodes = {
        ...
        if target == "job_operations" and node in result_nodes:
            return ["C11"]
        if target == "job_operations" and node == "clean_slab_result_ready":
            return ["C12.2"]
        if target == "adsorption_monitoring" and node in result_nodes | {"source_filter", "result_ready"}:
            return ["C12.6"]
        if target == "adsorption_submission" and node in adsorption_submission_nodes:
            return ["C12.6"]
        return None
    ...
    def _review_label(review_type: str) -> str:
        labels = {
            ...
            "result_download_review_required": "等待 DFT 结果下载确认",
            "adsorption_intermediate_review_required": "C12.1 等待选择吸附中间体",
            "adsorption_structure_review_required": "C12.4 等待吸附结构审查",
            "adsorption_dft_input_review_required": "C12.5 等待吸附 VASP 输入审查",
            "adsorption_dft_execution_required": "C12.6 等待选择吸附 DFT 执行方式",
            "adsorption_energy_review_required": "C12.7 等待吸附能审查",
        }
